...
var playlist: Dictionary = {
	PlaylistTheme.PEACE: [preload("res://assets/music/main_music.ogg"), preload("res://assets/music/forest_biome.ogg")],
...
func play_theme(theme: int, repeat_themes: bool = true) -> void:
	if current_theme != theme or !stream_player.playing:
		is_repeating = false
		stream_player.stop()
		...
		if theme_tracks != []:
			stream_player.stream = theme_tracks[randi() % theme_tracks.size()]
			stream_player.play()
